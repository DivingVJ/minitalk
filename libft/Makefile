NAME = libft.a
CC = gcc
CFLAG = -Wall -Wextra -Werror

SRC =	ft_atoi.c ft_bzero.c ft_calloc.c ft_isalnum.c ft_isalpha.c \
		ft_isascii.c ft_isdigit.c ft_isprint.c ft_itoa.c ft_memchr.c \
		ft_memcpy.c ft_memmove.c ft_memset.c ft_putchar_fd.c \
		ft_putendl_fd.c ft_putnbr_fd.c ft_putstr_fd.c ft_strchr.c \
		ft_striteri.c ft_strjoin.c ft_strlcat.c ft_strlcpy.c ft_split.c \
		ft_strmapi.c ft_strncmp.c ft_strnstr.c ft_strrchr.c ft_substr.c \
		ft_tolower.c ft_toupper.c ft_strdup.c ft_memcmp.c ft_strlen.c\
		ft_strtrim.c ft_putnbr_b_fd.c ft_putunsignbr_fd.c ft_printf.c

BONUS_SRC =	ft_lstnew.c ft_lstadd_front.c ft_lstsize.c ft_lstadd_back.c \
			ft_lstlast.c ft_lstdelone.c ft_lstclear.c ft_lstiter.c \
			ft_lstmap.c

OBJ = $(patsubst %.c, %.o, $(SRC))
BONUS_OBJ = $(patsubst %.c, %.o, $(BONUS_SRC))
DEP = $(patsubst %.a, %.h, $(NAME))

all: $(NAME)

$(NAME): $(OBJ)
	ar rcs $(NAME) $(OBJ)

$(OBJ): $(SRC) $(DEP)
	$(CC) $(CFLAG) -c $(SRC)

bonus: $(BONUS_OBJ)
	ar rcs $(NAME) $(BONUS_OBJ)

$(BONUS_OBJ): $(BONUS_SRC) $(DEP)
	$(CC) $(CFLAG) -c $(BONUS_SRC)


clean:
	rm -f $(OBJ) $(BONUS_OBJ)

fclean: clean	
	rm -f $(NAME)

re: fclean all

.PHONY: clean fclean re all

